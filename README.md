...
- Als geen expliciete `internal_gains_forecast` wordt meegegeven, kan de runtime automatisch een persistente **`baseload_forecast`** afleiden uit historische telemetry + de laatste weerforecast; de optimizer vertaalt die vervolgens fysisch naar `Q_int` via `max(internal_gains_kw, internal_gains_heat_fraction × baseload_forecast)`, zodat slechts één achtergrondparameter hoeft te worden getuned
...
`RunRequest` ondersteunt naast de scalar `shutter_living_room_pct` ook een optionele `shutter_forecast: list[float]` met lengte `N`; deze array overschrijft de scalar fallback stap-voor-stap voor de UFH-zoninstraling. Daarnaast bestaan nu `baseload_forecast: list[float]`, `internal_gains_forecast: list[float]` en `internal_gains_heat_fraction`; als alleen `baseload_forecast` beschikbaar is, gebruikt de UFH-forecast deze samen met de baseline `internal_gains_kw` om een fysisch plausibeler tijdsvariabele `Q_int` op te bouwen, waarbij de effectieve elektrische referentievloer intern uit die baseline wordt afgeleid in plaats van apart te worden getuned.
