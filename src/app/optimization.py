...
from features.nowcaster import ForecastNowcaster
...
class Optimization:
    def __init__(self, state_manager: StateManager, models_path: Path) -> None:
    ...
    def run(self) -> None:
        ...
        nowcaster = ForecastNowcaster(
            alpha=0.7,
            max=mpc_config.boiler_power,
        )
        solar_trajectory = nowcaster.adjust(
            forecast=solar_forecast,
            actual_now=actual_now,
        )

        target_temps = tuple(
            float(p.value) for p in state.schedule.heat_pump.boiler.target_temperature
        )
