...
const forecastSeriesStyles = {
  gti_pv: { label: "GTI PV", color: "#f9a825" },
...
async function pollImportJob(jobId) {
  while (true) {
    ...
    if (payload.status === "succeeded") {
      status.className = "status success";
      status.textContent =
        `Import voltooid: ${payload.total_rows} rijen over ${payload.sensor_count} sensoren.`;
      return;
    }
...
async function loadCharts() {
  ...
  renderHeatpumpPowerPlot(
    ...
    {
      xRange: [startIso, endIso],
      loadSeries: [payload.baseload, payload.pv_output_power],
      loadTraceOptions: [
        { label: "Baseload", color: "#d32f2f" },
        { label: "PV opbrengst", color: "#f9a825" },
      ],
  ...
  renderPlot(
    supplyChart,
    [
      payload.hp_supply_target_temperature,
      payload.hp_supply_temperature,
      payload.hp_return_temperature,
      payload.hp_delta_t,
    ],
    {
...
function renderHeatpumpPowerPlot(element, powerSeries, modeSeries, statusSeriesList, options = {}) {
  ...
  if (options.loadSeries && Array.isArray(options.loadSeries)) {
    ...
    options.loadSeries.forEach((series, index) => {
      traces.push({
        x: series.points.map((point) => chartTimestamp(point.timestamp)),
        y: series.points.map((point) => point.value),
        name: loadTraceOptions[index]?.label || series.name,
        type: "scatter",
        mode: "lines",
        line: {
          color: loadTraceOptions[index]?.color || loadColors[index % loadColors.length],
          width: loadTraceOptions[index]?.width || 2,
  ...
  const modeIntervalsList = modeIntervals(modeSeries.points);
  const statusIntervalsByName = heatpumpStatusIntervals(statusSeriesList);
  traces.push(...heatpumpModeLegendTraces(modeIntervalsList));
  ...
  const mergedOptions = {
    ...
    shapes: [
      ...heatpumpModeShapes(modeIntervalsList),
...
function heatpumpStatusLegendTraces(statusIntervalsByName) {
  return statusIntervalsByName
    ...
    .map((series) => {
      const style = heatpumpStatusStyles[series.name] || { label: series.name, color: "#5c6bc0" };
      return {
        x: [null],
        y: [null],
        name: style.label,
...
function modeIntervals(modePoints) {
  ...
  for (let index = 1; index < modePoints.length; index += 1) {
    const point = modePoints[index];
...
function heatpumpModeLegendTraces(intervals) {
  const activeModes = Array.from(new Set(intervals.map((interval) => interval.mode)));
  return activeModes
...
function summarizeHeatpump(powerSeries, modeSeries, statusSeriesList) {
  ...
  const latest = latestPoint(powerSeries);
  ...
  return [`${latest.value.toFixed(1)} ${unit}`, displayMode(mode), ...statusLabels].join(" · ").trim();
}
