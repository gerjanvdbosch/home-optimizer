...
class IntentAwareMpcSolver:
    ...
    def solve(self, problem: IntentAwareMpcProblem) -> MpcPlan:
        pyo = _load_pyomo()
        annotated_problem = problem.model_copy(update={"horizon": self.annotate_horizon(problem)})
        self._current_intent_problem = annotated_problem
        try:
            model = self._build_model(annotated_problem, pyo)
            solver = self._build_solver(pyo, annotated_problem.max_solver_seconds)
            ...
            return self._plan_from_solution(
    ...
    @staticmethod
    def annotate_horizon(problem: IntentAwareMpcProblem) -> list[MpcHorizonStep]:
        intents_by_id = {
            intent.intent_id: intent
            for intent in (problem.run_intent_plan.intents if problem.run_intent_plan else [])
        }
        annotated_horizon: list[MpcHorizonStep] = []
        for index, (step, target) in enumerate(
            ...
            if intent is None:
                ...
                per_step_target = intent.target_charge_kwh / max(len(window_indices), 1)
                preheat_budget_share_kwh = per_step_target if index in window_indices else 0.0
                if index in window_indices:
                    preheat_block_cumulative_target_kwh = per_step_target * (
                        window_indices.index(index) + 1
                    )
                else:
                    preheat_block_cumulative_target_kwh = 0.0
                preheat_block_budget_kwh = intent.target_charge_kwh
                preheat_block_id = intent.source_block_id
            annotated_horizon.append(
                step.model_copy(
                    update={
                        ...
                        "preheat_block_id": preheat_block_id,
        ...
        for index, horizon_step in enumerate(problem.horizon):
            ...
            steps.append(
                MpcPlanStep(
                    ...
                    mass_temp_c=(
                        float(pyo.value(model.mass_temp[index]))
                        if hasattr(model, "mass_temp")
                        else None
                    ),
    ...
    def _build_model(self, problem: IntentAwareMpcProblem, pyo: Any) -> Any:
    ...
    def _build_linear_model(self, problem: IntentAwareMpcProblem, pyo: Any) -> Any:
        model = pyo.ConcreteModel(name="intent_aware_space_heating_mpc")
        ...
        model.initial_hp_on = pyo.Param(initialize=1 if problem.initial_state.hp_on else 0)
        ...
        self._apply_common_constraints(model=model, problem=problem, pyo=pyo)
        self._apply_common_objective(model=model, problem=problem, pyo=pyo)
        return model

    def _build_2state_model(self, problem: IntentAwareMpcProblem, pyo: Any) -> Any:
        model = pyo.ConcreteModel(name="intent_aware_space_heating_mpc_2state")
        horizon_size = len(problem.horizon)
        objective_context = self._build_objective_context(problem)
        model.T = pyo.RangeSet(0, horizon_size - 1)
        model.T_transition = pyo.RangeSet(0, max(0, horizon_size - 2))
    ...
    def _apply_common_constraints(
        ...
        *,
        ...
        def comfort_low_rule(model_ref: Any, t: int) -> Any:
            return problem.horizon[t].temp_min_c - model_ref.slack_low[t] <= model_ref.room_temp[t]

        def comfort_high_rule(model_ref: Any, t: int) -> Any:
            return model_ref.room_temp[t] <= problem.horizon[t].temp_max_c + model_ref.slack_high[t]
        ...
        self._apply_intent_start_limits(model=model, pyo=pyo)
        self._apply_intent_budget_constraints(model=model, pyo=pyo)
    ...
    def _apply_common_objective(
        ...
        *,
        ...
        captured_pv_kwh_term = sum(model.preheat_charge[t] for t in range(horizon_size))
        runtime_term = sum(
        ...
        preheat_budget_shortfall_term = self._intent_budget_shortfall_expression(
            model=model,
            problem=problem,
            pyo=pyo,
        )
    ...
    def _build_objective_context(self, problem: IntentAwareMpcProblem) -> _ObjectiveContext:
        ...
        q_heat_eff_big_m_kw = max(
            (float(step.effective_heating_kw_forecast) for step in problem.horizon),
            default=0.0,
        )
    ...
    def _apply_intent_start_limits(self, *, model: Any, pyo: Any) -> None:
    ...
    def _apply_intent_budget_constraints(self, *, model: Any, pyo: Any) -> None:
        intent_problem = self._current_intent_problem
        if intent_problem is None or intent_problem.run_intent_plan is None:
    ...
    @staticmethod
    def _intent_budget_shortfall_expression(
        *,
        model: Any,
        problem: IntentAwareMpcProblem,
        pyo: Any,
