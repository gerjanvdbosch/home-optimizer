...
class OpenMeteoForecastService:
    def __init__(
        ...
        *,
        ...
        living_room_window_tilt: float | None,
        ...
        self.living_room_window_tilt = living_room_window_tilt
        self.living_room_window_azimuth = living_room_window_azimuth
        self.forecast_steps = forecast_steps

    @property
    def enabled(self) -> bool:
        return self._enabled and self.latitude is not None and self.longitude is not None

    def refresh_forecast(self, created_at: datetime | None = None) -> int:
        if not self.enabled:
            LOGGER.info("Open-Meteo forecast refresh skipped: configuration incomplete or disabled")
            return 0
    ...
    def _build_gti_entries(
        ...
        if (
            self.living_room_window_tilt is not None
            and self.living_room_window_azimuth is not None
        ):
            payload = self.gateway.fetch_minutely_forecast(
                latitude=latitude,
                longitude=longitude,
                variables=[GTI_VARIABLE],
                forecast_steps=self.forecast_steps,
                tilt=self.living_room_window_tilt,
