...
def _build_training_matrix(
    ...
    for source_index in range(max_lag, len(rows) - 1):
        ...
        for field_name, _, lag in specs:
            ...
            if value is None:
                value = default_feature_value(field_name)
            if value is None:
                valid = False
                break
...
def predict_next_room_arx_temperature(
    ...
    *,
    ...
    prediction_origin_index = source_index if prediction_origin_index is None else prediction_origin_index

    feature_values: list[float] = []
    for field_name, _, lag in feature_specs(model.config):
        lagged_index = source_index - lag
...
def simulate_room_arx_horizon(
    ...
    *,
    ...
    for step in range(1, horizon_steps + 1):
        ...
        prediction = predict_next_room_arx_temperature(
            model,
            rows,
