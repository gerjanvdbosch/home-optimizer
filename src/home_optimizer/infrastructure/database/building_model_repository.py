...
from home_optimizer.domain import BuildingTemperatureModel, normalize_utc_timestamp
from home_optimizer.domain.time import parse_datetime
from home_optimizer.infrastructure.database.orm_models import BuildingTemperatureModelRecord
...
class BuildingModelRepository:
    ...
    def latest(self) -> BuildingTemperatureModel | None:
        with self.database.session() as session:
            stmt = (
                select(BuildingTemperatureModelRecord)
                .order_by(BuildingTemperatureModelRecord.trained_at_utc.desc())
                .limit(1)
            )
    ...
    @staticmethod
    def _to_record(model: BuildingTemperatureModel) -> BuildingTemperatureModelRecord:
        return BuildingTemperatureModelRecord(
    ...
    @staticmethod
    def _to_model(row: BuildingTemperatureModelRecord) -> BuildingTemperatureModel:
