...
def index(request: Request, explain: str = None, train: str = None, view: str = "hour"):
    ...
    details = {
        ...
        "Target power": f"{result['target_power']:.2f} kW" if result is not None else "0.00 kW",
        ...
        "Boiler SoC": f"{result['dhw_soc']*100:.1f} %" if result is not None else "0.0 %",
        "Boiler Energy": f"{result['dhw_energy_kwh']:.2f} kWh" if result is not None else "0.00 kWh",
...
def _get_thermal_plan_plot(request: Request) -> str:
    ...
    times = [start_time + timedelta(minutes=15 * i) for i in range(len(result["planned_room"]))]
