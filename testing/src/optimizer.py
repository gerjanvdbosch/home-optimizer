...
class ThermalMPC:
    ...
    def _get_targets(self, now, T):
        ...
        for t in range(T):
            ...
            d_min[t] = 49.0 if 20 <= h < 21 else 10.0
            d_max[t] = 55.0
    ...
    def solve(
        ...
        self.P_comp_on_init.value = 1.0 if state["hvac_mode"] in [
            HvacMode.HEATING.value, HvacMode.DHW.value
        ] else 0.0
        ...
        logger.info(f"[Debug] DHW demand totaal: {dhw_demand[:T].sum():.2f} K  max: {dhw_demand[:T].max():.2f} K")

        # ── Comfortboetes (dimensioneel correct) ─────────────────────────
        t_out_now   = float(forecast_df.temp.values[0])
        avg_cop_ufh = self.perf_map.predict_cop(HvacMode.HEATING.value, t_out_now, state["room_temp"])
        avg_cop_dhw = self.perf_map.predict_cop(HvacMode.DHW.value,     t_out_now, state["dhw_bottom"])
        ...
        for iteration in range(linearizer.max_iter):
            ...
            if status not in ("optimal", "optimal_inaccurate"):
                logger.warning(f"[SLP] Niet-optimale status ({status}) in iteratie {iteration}")
                break
...
class Optimizer:
    def __init__(self, config, database):
        ...
        self.res_ufh   = UfhResidualPredictor(config.ufh_model_path, self.ident.R, self.ident.C)
    ...
    def resolve(self, context: Context, avg_price: float = 0.22, export_price: float = 0.05):
        """
        ...
        if steps_remaining > 0:
            ...
            solar_self_rem = float(np.sum(self.mpc.p_solar_self.value[:sl]) * self.mpc.dt)
