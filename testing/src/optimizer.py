...
class SystemIdentificator:
    ...
    def train(self, df: pd.DataFrame):
        ...
        if len(train_data) < 50:
            logger.warning("[Identificatie] Te weinig stabiele data voor betrouwbare training.")
            return
...
class MLResidualPredictor:
    ...
    def __init__(self, path: str):
        self.path = Path(path)
        self.model = None
        self.features_ufh = ["temp", "solar", "wind", "hour_sin", "hour_cos", "doy_sin", "doy_cos"]
    ...
    def train(self, df: pd.DataFrame, R, C, cop_model, is_dhw=False):
        ...
        if not is_dhw:
            ...
            df["cop_calc"] = df.apply(lambda row: cop_model.get_cop(row["temp"], row["room_temp"] + 6.0), axis=1)
            ...
            df["cop_calc"] = df.apply(lambda row: cop_model.get_cop(row["temp"], row["dhw_temp"] + 9.0), axis=1)
...
# =========================================================
class ThermalMPC:
    def __init__(self, ident: SystemIdentificator, cop_ufh: COPModel, cop_dhw: COPModel):
        self.ident = ident
    ...
    def _build_problem(self):
        ...
        for t in range(T):
            ...
            net_load_expr = self.p_el_ufh[t] + self.p_el_dhw[t] + self.P_load_base[t] - self.P_solar_avail[t]
            constraints += [
        ...
        # Anti-Pendel
        switches = cp.sum(cp.abs(self.u_ufh[1:] - self.u_ufh[:-1])) + \
                   cp.sum(cp.abs(self.u_dhw[1:] - self.u_dhw[:-1]))

        # Comfort Penalties
        # 1. Te koud (Hard & Comfort)
        pen_room_cold = cp.sum(self.slack_room_low) * 25.0 + cp.sum(self.slack_room_comfort) * 0.6
        ...
        total_obj = cost - solar_bonus + switches + \
                    pen_room_cold + pen_dhw_cold + \
                    pen_room_hot + pen_dhw_hot + \
                    track_room + track_dhw

        self.problem = cp.Problem(cp.Minimize(total_obj), constraints)

        # Deze assert zou nu moeten slagen
        assert self.problem.is_dpp(), "Probleem is niet DPP compliant! Check parameters."
    ...
    def solve(self, state, forecast_df, prices, ufh_residuals, dhw_residuals):
        ...
        for t in range(T):
            ...
            # --- UFH Logic ---
            t_sink_ufh = sim_t_room + 6.0 # Schatting delta
            cop_u = self.cop_ufh_model.get_cop(temp_out, t_sink_ufh)
            v_cop_ufh.append(cop_u)

            # Minimaal vermogen stijgt bij erge kou
            correction = max(0, (0 - temp_out) * 0.05)
            v_min_p_ufh.append(0.8 + correction)

            # Update sim (eenvoudige Euler stap)
            rise_room = ((1.5 * cop_u) - (sim_t_room - temp_out)/self.ident.R) * dt/self.ident.C
            sim_t_room = min(sim_t_room + max(0, rise_room), 22.0)

            # --- DHW Logic ---
            t_sink_dhw = sim_t_dhw + 9.0 # Schatting delta
            cop_d = self.cop_dhw_model.get_cop(temp_out, t_sink_dhw)
            v_cop_dhw.append(cop_d)

            # Update sim
            rise_dhw = (3.2 * cop_d * dt) / 0.232
            sim_t_dhw = min(sim_t_dhw + rise_dhw, 60.0)
        ...
        mode = "DHW" if self.u_dhw.value[0] > 0.5 else "UFH" if self.u_ufh.value[0] > 0.5 else "OFF"
...
class Optimizer:
    ...
    def resolve(self, context: Context):
        ...
        prices = context.prices if hasattr(context, "prices") else [0.21] * len(horizon_df)
    ...
    def train(self, days_back: int = 730):
        ...
        self.ufh_res.train(history_df, self.ident.R, self.ident.C, self.cop_ufh, is_dhw=False)
        self.dhw_res.train(history_df, self.ident.R, self.ident.C, self.cop_dhw, is_dhw=True)
